...
bool on_scan_event(const char*ssid, esp_bd_addr_t address, int rrsi){
    Serial.print("Here: ");
    String addr = "";
    for (int i=0;i<8;i++){
      addr = addr + String(address[i]);
    }
    String message = "{\"ssid\": \"" + String(ssid) + "\", \"address\": \"" + addr + "\"}";
...
void scan(){
  Serial.println("Calling Updated start_speaker");
  a2dp_source.set_auto_reconnect(false);
  a2dp_source.set_ssid_callback(on_scan_event);
  a2dp_source.start_raw("", on_data_requested); 
}
...
void handle_commands(){
  ...
  if (num_bytes_available){
    ...
    if (num_bytes_available>6){
      ...
      }else if(readable_cmd.substring(2,6).equals("scan")){
